...
func (s *Store) migrate() error {
	stmts := []string{
		...
		`CREATE TABLE IF NOT EXISTS orchestrator_sessions (
			...
			guardrail_paused INTEGER NOT NULL DEFAULT 0,
			guardrail_reason TEXT NOT NULL DEFAULT '',
			max_jobs INTEGER NOT NULL DEFAULT 60,
		...
		`CREATE TABLE IF NOT EXISTS orchestrator_messages (
			...
			target_pane TEXT NOT NULL DEFAULT 'all',
		...
		`CREATE TABLE IF NOT EXISTS orchestrator_job_links (
		...
		`CREATE INDEX IF NOT EXISTS idx_orchestrator_sessions_project_created ON orchestrator_sessions(project_id, created_at);`,
		`CREATE INDEX IF NOT EXISTS idx_orchestrator_messages_session_cursor ON orchestrator_messages(session_id, cursor);`,
		`CREATE INDEX IF NOT EXISTS idx_orchestrator_job_links_session_updated ON orchestrator_job_links(session_id, updated_at);`,
		`CREATE INDEX IF NOT EXISTS idx_orchestrator_job_links_session_job_id ON orchestrator_job_links(session_id, job_id);`,
